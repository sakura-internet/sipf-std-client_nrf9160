...
LOG_MODULE_REGISTER(download_client, CONFIG_DOWNLOAD_CLIENT_AUTH_LOG_LEVEL);
...
static int socket_timeout_set(int fd, int type)
{
	...
	if (type == SOCK_STREAM) {
		timeout_ms = CONFIG_DOWNLOAD_CLIENT_AUTH_TCP_SOCK_TIMEO_MS;
	} else {
		timeout_ms = CONFIG_DOWNLOAD_CLIENT_AUTH_UDP_SOCK_TIMEO_MS;
	}
...
{
	__ASSERT(client->offset <= CONFIG_DOWNLOAD_CLIENT_AUTH_BUF_SIZE,
		 "Buffer overflow!");
			    ...
			    const struct download_client_cfg *config)
{
	...
	if (config->frag_size_override > CONFIG_DOWNLOAD_CLIENT_AUTH_BUF_SIZE) {
	...
	if (IS_ENABLED(CONFIG_DOWNLOAD_CLIENT_AUTH_IPV6)) {
		err = host_lookup(host, AF_INET6, config->pdn_id, config->apn, &sa);
	}
	if (err || !IS_ENABLED(CONFIG_DOWNLOAD_CLIENT_AUTH_IPV6)) {
