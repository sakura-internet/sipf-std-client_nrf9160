...
#include <net/download_client_auth.h>

LOG_MODULE_DECLARE(download_client, CONFIG_DOWNLOAD_CLIENT_AUTH_LOG_LEVEL);

#define HOSTNAME_SIZE CONFIG_DOWNLOAD_CLIENT_AUTH_MAX_HOSTNAME_SIZE
#define FILENAME_SIZE CONFIG_DOWNLOAD_CLIENT_AUTH_MAX_FILENAME_SIZE
...
int download_client_create_authorization(const char *user_name, const char *password, char *auth_buff, int sz_auth_buff)
{
	char user_pass[128];
	int sz_out;

	int sz = sprintf(user_pass, "%s:%s", user_name, password);
	if (base64_encode(auth_buff, sz_auth_buff, &sz_out, user_pass, sz) != 0) {
...
int http_get_request_send(struct download_client *client)
{
	...
	if (client->config.frag_size_override) {
		...
		off = client->progress +
			CONFIG_DOWNLOAD_CLIENT_AUTH_HTTP_FRAG_SIZE - 1;
	}
	...
	if (client->proto == IPPROTO_TLS_1_2
	   || IS_ENABLED(CONFIG_DOWNLOAD_CLIENT_AUTH_RANGE_REQUESTS)) {
		if (client->config.authorization == NULL) {
			len = snprintf(client->buf,
				CONFIG_DOWNLOAD_CLIENT_AUTH_BUF_SIZE,
				HTTP_GET_RANGE, file, host, client->progress, off);			
		} else {
			len = snprintf(client->buf,
				CONFIG_DOWNLOAD_CLIENT_AUTH_BUF_SIZE,
				HTTP_GET_RANGE_AUTH, file, host, client->progress, off, client->config.authorization);
		}
		...
		if (client->config.authorization == NULL) {
			...
			len = snprintf(client->buf,
				CONFIG_DOWNLOAD_CLIENT_AUTH_BUF_SIZE,
				HTTP_GET_OFFSET_AUTH, file, host, client->progress, client->config.authorization);
		}
	} else {
		if (client->config.authorization == NULL) {
			len = snprintf(client->buf,
			...
			len = snprintf(client->buf,
				CONFIG_DOWNLOAD_CLIENT_AUTH_BUF_SIZE,
				HTTP_GET_AUTH, file, host, client->config.authorization);			
		}
	}

	if (len < 0 || len > CONFIG_DOWNLOAD_CLIENT_AUTH_BUF_SIZE) {
		LOG_ERR("Cannot create GET request, buffer too small");
		return -ENOMEM;
	}

	if (IS_ENABLED(CONFIG_DOWNLOAD_CLIENT_AUTH_LOG_HEADERS)) {
...
int http_parse(struct download_client *client, size_t len)
{
	...
	if (client->progress != client->file_size &&
	    client->offset < (client->config.frag_size_override != 0 ?
			      ...
			      CONFIG_DOWNLOAD_CLIENT_AUTH_HTTP_FRAG_SIZE)) {
		return 1;
	}
