...
#define FILE_BUF_LEN ((CONFIG_DOWNLOAD_CLIENT_AUTH_MAX_FILENAME_SIZE*2)+1)
#else
#define FILE_BUF_LEN (CONFIG_DOWNLOAD_CLIENT_AUTH_MAX_FILENAME_SIZE)
#endif

LOG_MODULE_REGISTER(fota_download, CONFIG_FOTA_DOWNLOAD_AUTH_LOG_LEVEL);
...
static uint8_t mcuboot_buf[CONFIG_FOTA_DOWNLOAD_AUTH_MCUBOOT_FLASH_BUF_SZ] __aligned(4);
#endif
...
{
#ifdef CONFIG_FOTA_DOWNLOAD_AUTH_PROGRESS_EVT
...
static int download_client_callback(const struct download_client_evt *event)
{
	...
	case DOWNLOAD_CLIENT_EVT_FRAGMENT: {
		...
		if (IS_ENABLED(CONFIG_FOTA_DOWNLOAD_AUTH_PROGRESS_EVT) &&
		    !first_fragment) {
			err = dfu_target_offset_get(&offset);
			if (err != 0) {
