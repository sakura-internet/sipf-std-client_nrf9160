...
struct download_client_cfg {
	...
	/** HTTP Authorization information.
	 *  Set a null-terminated string with the Authorization
	 *  or NULL to unuse Authorization header.
	 */
...
struct download_client {
	...
	int fd;
	/** Response buffer. */
	char buf[CONFIG_DOWNLOAD_CLIENT_AUTH_BUF_SIZE];
	/** Buffer offset. */
	size_t offset;
	...
	K_THREAD_STACK_MEMBER(thread_stack,
			      CONFIG_DOWNLOAD_CLIENT_AUTH_STACK_SIZE);
 ...
 * @kconfig{CONFIG_DOWNLOAD_CLIENT_AUTH_HTTP_FRAG_SIZE} bytes for HTTP, or
 * @kconfig{CONFIG_DOWNLOAD_CLIENT_AUTH_COAP_BLOCK_SIZE} bytes for CoAP,
